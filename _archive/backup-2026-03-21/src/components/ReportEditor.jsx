...
function ReportEditor({job, user, onBack, reportDate}){
  ...
  const toggleContractor=(name)=>{setSelectedContractors(p=>{const exists=p.find(c=>c.company_name===name);if(exists)return p.filter(c=>c.company_name!==name);return[...p,{company_name:name,manpower:0}];});};
  ...
  // ── TYR v3: Draw contractor table on canvas preview ──
  const drawContractorTableOnCanvas=(ctx,scale,pageH)=>{
    if(!isTYR||selectedContractors.length===0)return;
    // Find the "Inspection Notes" / contractor table area from field_config
    // Look for a field with "contractor" or "inspection notes" in the name for position reference
    const fc=job.field_config||{};
    const allF=[...(fc.editable||[]),...(fc.locked||[])];
    const refField=allF.find(f=>/contractor|inspection.?notes/i.test(f.name||""));
    // Default position if no reference field found (below header area on page 1)
    const startX=(refField?.x||36)*scale;
    const startY=(refField?.y||280)*scale;
    const rowH=14*scale;
    const colW=[140*scale,50*scale,80*scale,80*scale]; // Name, Manpower, RFIs, Submittals
    const sz=8*scale;
    ctx.font="bold "+sz+"px Georgia, Cambria, serif";
    ctx.fillStyle="#333333";
    // Header
    ctx.fillText("Contractor",startX+4*scale,startY+sz);
    ctx.fillText("Manpower",startX+colW[0]+4*scale,startY+sz);
    // Data rows
    ctx.font=sz+"px Georgia, Cambria, serif";
    ctx.fillStyle="#000000";
    selectedContractors.forEach((c,i)=>{
      const rowY=startY+rowH*(i+1);
      ctx.fillText(c.company_name,startX+4*scale,rowY+sz);
      ctx.fillText(String(c.manpower||0),startX+colW[0]+4*scale,rowY+sz);
    });
  };
  ...
  const viewReport=async()=>{
    ...
    try{
      ...
      for(let pi=1;pi<=pjDoc.numPages;pi++){
        ...
        // TYR v3: draw contractor table on page 1
        if(pi===1)drawContractorTableOnCanvas(ctx,scale,pageH);
  ...
  const submitReport=async()=>{
    ...
    try{
      ...
      // ── TYR v3: Draw contractor table on PDF page 1 ──
      if(isTYR&&selectedContractors.length>0&&pages.length>0){
        const pg1=pages[0];
        const pgH=pg1.getHeight();
        const allF=[...(fc.editable||[]),...(fc.locked||[])];
        const refF=allF.find(f=>/contractor|inspection.?notes/i.test(f.name||""));
        const tblX=refF?.x||36;
        const tblY=refF?.y||280;
        const rowH=12;
        const sz=8;
        const usedFont=font;
        // Header
        const hdrY=pgH-tblY-sz;
        pg1.drawText("Contractor",{x:tblX+2,y:hdrY,size:sz,font:usedFont,color:rgb(0.2,0.2,0.2)});
        pg1.drawText("Manpower",{x:tblX+142,y:hdrY,size:sz,font:usedFont,color:rgb(0.2,0.2,0.2)});
        // Data rows
        selectedContractors.forEach((c,i)=>{
          const rowY=hdrY-rowH*(i+1);
          pg1.drawText(c.company_name,{x:tblX+2,y:rowY,size:sz,font:usedFont,color:rgb(0,0,0)});
          pg1.drawText(String(c.manpower||0),{x:tblX+142,y:rowY,size:sz,font:usedFont,color:rgb(0,0,0)});
        });
      }
  ...
  return(
    <div className="page-in" style={{minHeight:"100vh",background:C.bg,color:C.txt}}>
          ...
          {editFields.filter(f=>!(f.autoFill==="date"||f.autoFill==="increment"||f.autoFill==="name")).map(f=>{
            ...
            <div key={f.name} style={{marginBottom:14,opacity:isSkipped?0.4:1,transition:"opacity 0.2s"}}>
              ...
              {!isSkipped&&((()=>{
                ...
                const useTextarea=isTYR?(f.voiceEnabled||f.multiline||isTyrActivities):(f.voiceEnabled||f.multiline||isNotesField);
                return useTextarea?(
      ...
      {!fieldMode&&<div style={{maxWidth:600,margin:"0 auto",padding:"20px 20px 140px"}}>
        ...
        {/* ── TYR v3: Contractor Picker (only for TYR jobs) ── */}
        {isTYR&&jobContractors.length>0&&(
          <div style={{marginBottom:20}}>
            <div style={{fontSize:12,fontWeight:700,color:C.org,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Contractors On Site</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {jobContractors.map(c=>{
                const sel=selectedContractors.find(s=>s.company_name===c.company_name);
                return(
                  <div key={c.id} style={{display:"flex",flexDirection:"column",gap:4}}>
                    <button onClick={()=>toggleContractor(c.company_name)} style={{padding:"8px 14px",borderRadius:8,border:`1px solid ${sel?C.org:C.brd}`,background:sel?C.org+"22":C.inp,color:sel?C.org:C.lt,fontSize:13,fontWeight:600,cursor:"pointer",transition:"all 0.15s"}}>
                      {sel?"✓ ":""}{c.company_name}{c.trade?` (${c.trade})`:""}
                    </button>
                    {sel&&(
                      <div style={{display:"flex",alignItems:"center",gap:6,justifyContent:"center"}}>
                        <button onClick={()=>updateContractorManpower(c.company_name,Math.max(0,(sel.manpower||0)-1))} style={{width:24,height:24,borderRadius:6,border:`1px solid ${C.brd}`,background:C.inp,color:C.lt,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                        <span style={{fontSize:13,fontWeight:600,color:C.lt,minWidth:20,textAlign:"center"}}>{sel.manpower||0}</span>
                        <button onClick={()=>updateContractorManpower(c.company_name,(sel.manpower||0)+1)} style={{width:24,height:24,borderRadius:6,border:`1px solid ${C.brd}`,background:C.inp,color:C.lt,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                        <span style={{fontSize:10,color:C.mut}}>crew</span>
                      </div>
                    )}
                  </div>
                );

...
function Dashboard({user, onLogout}){
  ...
  // Paywall screen — DISABLED for now (payment UI hidden until post-launch)
  if(false&&(subStatus==="expired"||subStatus==="cancelled"||subStatus==="trial_abused")){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.txt,position:"relative"}} onClick={e=>{if(showSet&&!e.target.closest('[data-s]'))setShowSet(false);if(showDD&&!e.target.closest('[data-d]'))setShowDD(false);}}>
      ...
      {/* Trial countdown banner — HIDDEN until post-launch */}
      {false&&subStatus==="trialing"&&trialEndsAt&&(()=>{
        const days=Math.max(0,Math.ceil((trialEndsAt-new Date())/(1000*60*60*24)));
        return days<=7?(
          <div style={{padding:"10px 16px",background:days<=3?"rgba(239,68,68,0.15)":"rgba(232,116,42,0.12)",borderBottom:`1px solid ${C.brd}`,display:"flex",alignItems:"center",justifyContent:"space-between",maxWidth:600,margin:"0 auto"}}>
      ...
      {/* Job dropdown — centered on desktop */}
      {showDD&&(
        <div style={{position:"absolute",top:62,left:0,right:0,display:"flex",justifyContent:"flex-start",zIndex:90,pointerEvents:"none"}} data-d>
        <div style={{maxWidth:600,width:"100%",margin:"0 auto",paddingLeft:16,pointerEvents:"none"}}>
        <div style={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:12,marginTop:4,boxShadow:"0 8px 24px rgba(0,0,0,0.5)",maxHeight:400,overflowY:"auto",minWidth:240,display:"inline-block",pointerEvents:"auto"}}>
      ...
      {/* Settings dropdown — centered on desktop */}
      {showSet&&(
        <div style={{position:"absolute",top:62,right:0,left:0,display:"flex",justifyContent:"flex-end",zIndex:100,pointerEvents:"none"}} data-s>
        <div style={{maxWidth:600,width:"100%",margin:"0 auto",display:"flex",justifyContent:"flex-end",paddingRight:16,pointerEvents:"none"}}>
        <div style={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:12,padding:"8px 0",minWidth:220,boxShadow:"0 8px 24px rgba(0,0,0,0.5)",pointerEvents:"auto"}}>
